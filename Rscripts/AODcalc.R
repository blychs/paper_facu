...
ggplot()+  geom_point(data = CEILAP_BA_hora2, 
  ...
  # geom_point(data = CEILAP_BA_hora2[(CEILAP_BA_hora2$Event_F %in% c("SI")) & (CEILAP_BA_hora2$AOD_440nm<0.16) ,], 
  #            aes(x=AOD_440nm,y=`440-870_Angstrom_Exponent`,color=OrigenTag), shape=21, size=2)+
  # geom_hline(yintercept=1.13,color="grey50")+
  ...
  # scale_x_continuous(breaks = function(x) unique(c(0.1613, scales::pretty_breaks()(x))),
  #                    labels = function(x) format(x, nsmall = 2, digits = 2))
dev.off()
ggplot()+ geom_point(data = CEILAP_BA_hora2, 
                     aes(x=AOD_440nm,y=`440-870_Angstrom_Exponent`))+
