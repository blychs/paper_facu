...
ggplot(PMF_BA_full)+
  ...
  # geom_line(aes(x=date, y=Ca/40,color="Ca"))+
...
ggplot(PMF_BA_full)+
  geom_line(aes(x=date, y=Kmolar,color="K"))+
  geom_line(aes(x=date, y=Ca/40,color="Ca"))+
  geom_line(aes(x=date, y=Mg/24,color="Mg"))+  
  # geom_line(aes(x=date, y=NH4molar,color="NH4"))+
  # geom_line(aes(x=date, y=NO3molar,color="NO3"))+
  # geom_line(aes(x=date, y=SO4molar,color="SO4"))+
  geom_point(data=PMF_BA_full[PMF_BA_full$Event_F %in% c("SI","SF"),],aes(x=date, y=NH4molar),color="black",show.legend = FALSE)
...
#molar ratio version review ####
library(ggplot2)
library(patchwork)

# Definir el rango de fechas de eventos para sombrear
event_periods <- PMF_BA_full %>%
  filter(Event_F %in% c("SI", "SF", "SO")) %>%
  summarise(start = min(date), end = max(date))

plot1 <- ggplot(PMF_BA_full) + 
  # Sombrear períodos de eventos con un fondo naranja translúcido
  geom_rect(data = PMF_BA_full[PMF_BA_full$Event_F %in% c("SI", "SF", "SO"), ], 
            aes(xmin = date, xmax = date + 86400, ymin = -Inf, ymax = Inf,fill = "BB-events"), alpha = 0.4, inherit.aes = FALSE)+
  scale_fill_manual(name = "", values = c("BB-events" = "orange"))+
  geom_line(aes(x = date, y = NH4SO4, color = "NH4/SO4"), linetype = "dashed") +
  geom_line(aes(x = date, y = NH42SO4NO3, color = "NH4/(2SO4+NO3)"), linetype = "dashed") +
  geom_point(aes(x = date, y = NH4SO4, color = "NH4/SO4"), shape = 16, size = 2) +
  geom_point(aes(x = date, y = NH42SO4NO3, color = "NH4/(2SO4+NO3)"), shape = 16, size = 2) +
  geom_abline(aes(slope = 0, intercept = i1), linetype = "dashed") +
  geom_abline(aes(slope = 0, intercept = i2), color = 'gray20', linetype = "dashed") +
  labs(y = "Molar ratio", x = "") +
  scale_color_discrete(name = "Molar ratios", na.translate = FALSE) +
  ...
  annotate("text", x = min(PMF_BA_full$date), y = Inf, label = "(a)", 
           vjust = 1.5, hjust = 1.4, size = 5, fontface = "bold") +
  scale_x_datetime(expand = c(0, 0), date_labels = "%m-%Y", date_breaks = "2 month")

plot2 <- ggplot(PMF_BA_full) + 
  # Sombrear períodos de eventos con fondo naranja translúcido
  geom_rect(data = PMF_BA_full[PMF_BA_full$Event_F %in% c("SI", "SF", "SO"), ], 
            aes(xmin = date, xmax = date + 86400, ymin = -Inf, ymax = Inf,fill = "BB-events"), 
             alpha = 0.4, inherit.aes = FALSE) +
  scale_fill_manual(name = "", values = c("BB-events" = "orange"))+
  geom_line(aes(x = date, y = NH4, color = "NH4"), linetype = "dashed") +
  geom_line(aes(x = date, y = NO3, color = "NO3"), linetype = "dashed") +
  geom_line(aes(x = date, y = SO4, color = "SO4"), linetype = "dashed") +
  geom_point(aes(x = date, y = NH4, color = "NH4"), shape = 16, size = 2) +
  geom_point(aes(x = date, y = NO3, color = "NO3"), shape = 16, size = 2) +
  geom_point(aes(x = date, y = SO4, color = "SO4"), shape = 16, size = 2) +
  labs(y = expression(paste("Concentration [", mu, "g/m"^3, "]")), x = "") +
  scale_color_discrete(name = "Ions concentration", na.translate = FALSE) +
  guides(color = guide_legend(nrow = 1)) + 
  scale_y_continuous(
...
print(combined_plot)
# Guardar la imagen
png(paste0("../", pathgraphs, "/ionesratiosreviewlineas.png"), width = 550 * 5, height = 550 * 5, res = 300)
print(combined_plot)
dev.off()
...
# gases ####
gases20192020 <- read_csv("/media/usuario/32b62ac8-a81b-4630-bf83-822c71ee0cad/mdiaz/Documents/paper_facu/data/DatosCNEAgases/CNEA-CAC-AQdata20192020.csv")
gases20192020$date_puestadefiltro=gases20192020$date-12*3600
gases20192020$dateoriginal=gases20192020$date
gases20192020$date=gases20192020$date_puestadefiltro
gases20192020day=timeAverage(gases20192020, avg.time = "day")
mergeogasesyparticulas= merge(PMF_BA_full,gases20192020day, by="date")

# Pedidos de Julian ####
ggplot(mergeogasesyparticulas) + 
  geom_line(aes(x=date,y=300*SO2, color="SO2")) + 
  geom_line(aes(x=date,y=SO4, color= "SO4"))
# +
  # geom_line(aes(x=date,y=NO3, color= "NO3"))

ggplot(timeAverage(mergeogasesyparticulas, avg.time = "month")) + 
  geom_line(aes(x=date,y=300*SO2, color="SO2")) + 
  geom_line(aes(x=date,y=SO4, color= "SO4"))
# +
  # geom_line(aes(x=date,y=NO3, color= "NO3"))
...
ggplot(mergeogasesyparticulas) + 
  geom_point(aes(y=NO3_SO4,x=NH4_SO4)) + xlim(c(0,6))+ ylim(c(0,6))

mergeogasesyparticulas$periodo = "Desde octubre" # dividirlo por periodos
# mergeogasesyparticulas$periodo = mergeogasesyparticulas[]

t2=ggplot(mergeogasesyparticulas) + geom_line(aes(x=date,y=t2))
rh2=ggplot(mergeogasesyparticulas) + geom_line(aes(x=date,y=rh2))

library(ggpubr)
ggarrange(t2, rh2,  ncol = 1, nrow = 2)

t2=ggplot(timeAverage(mergeogasesyparticulas,avg.time = "month")) + geom_line(aes(x=date,y=t2))
rh2=ggplot(timeAverage(mergeogasesyparticulas,avg.time = "month")) + geom_line(aes(x=date,y=rh2))

library(ggpubr)
ggarrange(t2, rh2,  ncol = 1, nrow = 2)
ggplot(selectByDate(mergeogasesyparticulas,end = "2019-05-25")) + 
     geom_point(aes(y=NO3_SO4,x=NH4_SO4)) + xlim(c(0,6))+ ylim(c(0,6))

ggplot(selectByDate(mergeogasesyparticulas,start = "2019-05-25",end = "2019-10-01")) + 
  geom_point(aes(y=NO3_SO4,x=NH4_SO4,color=date)) + xlim(c(0,6))+ ylim(c(0,6))

ggplot(selectByDate(mergeogasesyparticulas,start = "2019-10-01")) + 
  geom_point(aes(y=NO3_SO4,x=NH4_SO4)) + xlim(c(0,6))+ ylim(c(0,6))

mergeogasesyparticulas$SO2_SO4=mergeogasesyparticulas$SO2/mergeogasesyparticulas$SO4
ggplot(mergeogasesyparticulas) + 
  geom_line(aes(x=date,y=SO2_SO4, color="SO2/SO4"))

ggplot(mergeogasesyparticulas) + 
  geom_point(aes(x=SO4,y=300*SO2, color="SO2"))
...
# NO3/SO4 vs NH4/SO4
mergeogasesyparticulas$NO3_SO4=mergeogasesyparticulas$NO3/MWNO3/(mergeogasesyparticulas$SO4/MWSO4)
mergeogasesyparticulas$NH4_SO4=mergeogasesyparticulas$NH4/MWNH4/(mergeogasesyparticulas$SO4/MWSO4)
ggplot(mergeogasesyparticulas) + 
  geom_point(aes(y=NO3_SO4,x=NH4_SO4)) + xlim(c(0,6))+ ylim(c(0,6))
ggplot(mergeogasesyparticulas) + 
  geom_point(aes(y=NO3,x=NH4)) + xlim(c(0,1))+ ylim(c(0,1))
...
ggplot()+geom_line(data=PMF_BA_full,aes(x=date,y=`PM2,5`))+
  geom_point(data=meteoobsday,aes(x=date,y=`PRECIP 6HS (mm)`, color="precip"))
ggplot() +
  geom_col(data = meteoobsday, aes(x = as.Date(date), y = `PRECIP 6HS (mm)`, fill = "precip")) +
  geom_line(data = PMF_BA_full, aes(x = as.Date(date), y = `PM2,5`)) +
  scale_x_date(limits = as.Date(c("2019-04-01", "2020-03-01"))) +
  coord_cartesian(expand = FALSE)
# Asegurar que los meses estén en inglés
Sys.setlocale("LC_TIME", "C")
library(dplyr)

PMF_BA_full <- PMF_BA_full %>%
  mutate(Event_S = case_when(
    Event_F %in% c("SI", "SF") ~ "BB sample",
    TRUE ~ "non-BB sample"
  ))

png(filename = "/media/usuario/32b62ac8-a81b-4630-bf83-822c71ee0cad/mdiaz/Documents/paper_facu/Figures/acumprecandpm.png",
    res=300, height=4*400,width=8.5*400)
ggplot() +
  geom_line(data = PMF_BA_full, aes(x = as.Date(date), y = `PM2,5`), color = "darkblue") +
  geom_point(data = PMF_BA_full, aes(x = as.Date(date), y = `PM2,5`, color = Event_S)) +
  geom_col(data = meteoobsday, aes(x = as.Date(date), y = `PRECIP 6HS (mm)` / (90 / 55)), 
           fill = "gray20", alpha = 0.6) +
  scale_x_date(name = "Date", limits = as.Date(c("2019-04-01", "2020-03-01")), date_labels = "%b %Y") +
  scale_y_continuous(
    name = "PM2.5 (µg/m³)", 
    limits = c(0, 55),
    sec.axis = sec_axis(~ . * (90 / 55), name = "Daily Accumulated Precipitation (mm)")
  ) +
  scale_color_manual(name="", values = c("BB sample" = "red",  "non-BB sample" = "darkblue"), na.value = "black") +
  coord_cartesian(expand = FALSE) +
  theme_bw() +
  theme(
    ...
    legend.position = c(0.9, 0.9), # Ubica la leyenda en la esquina superior derecha
    legend.text = element_text(size=16),
    legend.background = element_rect(fill = alpha("white", 0.7))
  )
dev.off()
TVmeteoobs=timeVariation(meteoobsday, pollutant = "PRECIP 6HS (mm)", statistic = "mean")
print(TVmeteoobs$plot$month)
meteoobsmonth=timeAverage(meteoobs[,c("date","PRECIP 6HS (mm)")],avg.time = "month",statistic = "sum")
ggplot()+geom_line(data=timeAverage(PMF_BA_full, avg.time = "month"),aes(x=date,y=`PM2,5`))+
...
ggplot(data, aes(x = date, y = rh)) +
 geom_point(color="darkblue") +
scale_x_datetime(date_breaks = "1 month", date_labels = "%b %Y") +  # Etiquetas "Ene 2024"
...
# Asegurar que date sea POSIXct
data$date <- as.POSIXct(data$date)

# Crear columna con el formato "YYYY-MM" para respetar el orden cronológico
data <- data %>%
  mutate(month_year = format(date, "%Y-%m"),
         month_year = factor(month_year, levels = sort(unique(month_year))))  # Ordenar correctamente

# Ajustar márgenes para que los espacios internos sean iguales
custom_theme <- theme(
  plot.margin = margin(10, 10, 10, 10),  # Márgenes iguales
  axis.text.x = element_text(angle = 0, hjust = 0.5)  # Etiquetas centradas
)

# Gráfico 1: Boxplot de Temperatura (verde claro)
p1 <- ggplot(data, aes(x = month_year, y = temp)) +
  geom_boxplot(fill = "lightgreen", color = "black") +
  labs(x = "", y = "Temperature (°C)") +
  theme_minimal() + custom_theme +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())  # Ocultar eje X

# Gráfico 2: Boxplot de Humedad Relativa (celeste)
p2 <- ggplot(data, aes(x = month_year, y = rh)) +  # RH = Humedad Relativa
  geom_boxplot(fill = "lightblue", color = "black") +
  labs(x = "Year-Month", y = "Relative Humidity (%)") +
  theme_minimal() + custom_theme

# Combinar con espacios internos iguales
(p1 / p2) + plot_layout(guides = 'collect', heights = c(1, 1))
# Combinar con espacios internos iguales
final_plot <- (p1 / p2) + plot_layout(guides = 'collect', heights = c(1, 1))

# Guardar en PNG con dpi 300
ggsave("boxplots_temperature_humidity.png", final_plot, width = 10, height = 6, dpi = 300)
...
monthPM= timeAverage(PMF_BA_full[,c("date","PM2,5")], avg.time = "month", statistic="mean")
...
# Gases
CNEA_CAC <- read_csv("/media/usuario/32b62ac8-a81b-4630-bf83-822c71ee0cad/mdiaz/Documents/paper_facu/data/DatosCNEAgases/CNEA-CAC-AQdata20192020.csv")
CNEA_CAC$date=CNEA_CAC$date-3600*12
CNEA_CACday=timeAverage(CNEA_CAC,avg.time = "day")
mergeblhPm2.5=merge(mergeblhPm2.5,CNEA_CAC, by="date")
ggplot(mergeblhPm2.5, aes(x=`PM2,5`,y=CO,color=Event_F))+geom_point()
ggplot(mergeblhPm2.5, aes(x=`PM2,5`,color=CO,y=blh))+geom_point()
