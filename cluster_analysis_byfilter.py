...
keys = ['PM2.5', 'Cl', 'NO3', 'SO4', 'Na$^+$', 'NH4', 'Na\nno sol',
...
ax.scatter(newmatrix['cluster'].apply(add_noise), newmatrix[element],
            marker='.', c=(newmatrix.index - np.timedelta64(12, 'h')), cmap='tab20b')
ax.set_xlabel("Cluster")
...
#%%
l_todrop = ["cluster", "ECPk1 C", "ECPk2 C", "ECPk3 C",
             "ECPk4 C", "ECPk5 C", "ECPk6 C", "TC",
             "OCPk1 C", "OCPk2 C", "OCPk3 C", "OCPk4 C",
             "OCPk5 C", "Pyrol C", "PM2.5", "Na total"]
...
pca_c1.loadings.plot.scatter(x="comp_00", y="comp_01", ax=ax, label="C#1",
                            marker='1')
pca_c2.loadings.plot.scatter(x="comp_00", y="comp_01", color='k',
                             ax=ax, label="C#2", marker='x')
pca_c3.loadings.plot.scatter(x="comp_00", y="comp_01", color='r',
                             ax=ax, label="C#3", marker='+')
pca_c4.loadings.plot.scatter(x="comp_00", y="comp_01", color='g',
                             ax=ax, label="C#4", marker="*")
pca_c5.loadings.plot.scatter(x="comp_00", y="comp_01", color='m',
                             ax=ax, label="C#5", marker='2')
ax.set_xlabel('PCA 1')
ax.set_ylabel('PCA 2')
plt.plot()

#%%
compare_PCA = pd.concat([pca_c1.factors["comp_00"], pca_c2.factors["comp_00"],
                          pca_c3.factors["comp_00"], pca_c4.factors["comp_00"],
                          pca_c5.factors["comp_00"]], axis=1)
...
my_cmap = mpl.colors.LinearSegmentedColormap.from_list("my_own_cmap",
                                    [(0,1,0.5), (1,0.5,0), (0,0,1),
                                      (0,0,0), (1,0,1)], N=5)
#display(pca_all.factors)
pca_all_withcluster = pd.concat([pca_all.loadings, newmatrix["cluster"]],
                                axis=1)
pca_all_withcluster.plot.scatter(x="comp_00", y="comp_01",
                                 c=pca_all_withcluster["cluster"],
                                 cmap=my_cmap)#,
plt.xlabel("PC 1")
plt.ylabel("PC 2")
plt.show()
