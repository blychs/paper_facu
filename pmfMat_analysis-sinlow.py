...
ax.errorbar(matrix.index.where(matrix['PM2.5'].notna()).dropna(), matrix['PM2.5'].dropna(),
            yerr=unc['PM2.5'].dropna(), capsize=2, capthick=1, color='k', marker='.', zorder=0)
ax.plot(matrix.index, matrix['PM2.5'].where(events['Event'].isin(
    ['S', 'SP', 'SN'])), 'o', color='r', label='Smoke')
...
def exceedance(target): return (
    (matrix['PM2.5'] > target).sum()/valid_filters * 100).round(2)
...
ax.scatter(matrix['VentCoef'], matrix['PM2.5'].where(
    events['Event'].isin(['S', 'SP', 'SN'])), label='Event')
...
axes = pd.plotting.scatter_matrix(
    matrix, alpha=0.2, figsize=(40, 40), diagonal="kde")
...
ax.scatter(events['AOD440'], events['Alpha'].where(
    events['Event'].isin(['S', 'SP', 'SN'])), label='Event')
...
ax.errorbar(matrix.index, matrix['PM2.5'], yerr=unc['PM2.5'], marker='.',
            linestyle='-', capsize=3, capthick=1, label='PM2.5', color='k')
ax.errorbar(matrix.index, mass[1]['organic_mass'], yerr=mass[3]['uorganic_mass'],
            marker='.', capsize=3, capthick=1, linestyle='-', label="Organic mass")
ax.errorbar(matrix.index, mass[1]['inorganic_ions'], yerr=mass[3]['uinorganic_ions'],
            marker='.', capsize=3, capthick=1, linestyle='-', label="Inorganic ions")
ax.errorbar(matrix.index, mass[1]['geological_minerals'], yerr=mass[3]['ugeological_minerals'],
            marker='.', capsize=3, capthick=1, linestyle='-', label="Geological minerals")
ax.errorbar(matrix.index, mass[1]['elemental_C'], yerr=mass[3]['uelemental_C'],
            marker='.', capsize=3, capthick=1, linestyle='-', label="Elemental carbon")
ax.errorbar(matrix.index, mass[1]['salt'], yerr=mass[3]['usalt'],
            marker='.', capsize=3, capthick=1, linestyle='-', label="Sea salt")
...
mass_Simon = mass_reconstruction_mod(
    matrix, unc, events=events, equation="Simon_2011")
mass_Hand = mass_reconstruction_mod(
    matrix, unc, events=events, equation="Hand_2011")
mass_Maenhaut = mass_reconstruction_mod(
    matrix, unc, events=events, equation="Maenhaut_2002")
...
mean_OM = np.array([mass['organic_mass'].mean(), select_events(
...
mean_EC = np.array([mass['elemental_C'].mean(), select_events(
    mass['elemental_C']).mean(), select_no_events(mass['elemental_C']).mean()])
mean_salt = np.array([mass['salt'].mean(), select_events(
    mass['salt']).mean(), select_no_events(mass['salt']).mean()])
ax.bar(categories, mean_OM, label='Organic mass')
ax.bar(categories, mean_II, bottom=mean_OM, label='Inorganic ions')
ax.bar(categories, mean_GM, bottom=mean_II +
       mean_OM, label='Geological minerals')
ax.bar(categories, mean_EC, bottom=mean_GM +
       mean_II+mean_OM, label='Elemental carbon')
ax.bar(categories, mean_salt, bottom=mean_GM +
       mean_II+mean_OM+mean_EC, label='Sea salt')
ax.legend()

print((total_reconst_mass /
      matrix['PM2.5']).loc[total_reconst_mass/matrix['PM2.5'] > 1.4].dropna())
...
mass_reconst["Others"] = (mass_Maenhaut[1]["others"] + mass_Simon[1]
                          ["others"])/2 + mass_Maenhaut[1]["trace_elements"]


mass_reconst_perc = mass_reconst.apply(
    lambda x: x/mass_reconst['Reconstructed mass'] * 100)
...
mass_reconst_Hand_perc = mass_reconst_Hand.apply(
    lambda x: x/mass_reconst_Hand['Reconstructed mass'] * 100)
...
mass_reconst_Simon_perc = mass_reconst_Simon.apply(
    lambda x: x/mass_reconst_Simon['Reconstructed mass'] * 100)
...
mass_reconst_Maenhaut_perc = mass_reconst_Maenhaut.apply(
    lambda x: x/mass_reconst_Maenhaut['Reconstructed mass'] * 100)
...
#    axs[i][j].plot(matrix.index, reconst.where(events['Alpha']>0.85), 'Xg', label='Alpha', zorder=3)
    axs[i][j].plot(matrix.index, reconst.where(
        events['Event'] == "S"), 's', label='Smoke', zorder=1)
    axs[i][j].plot(matrix.index, reconst.where(events['Alpha'] > 0.85).where(
        events['AOD440'] > 0.185), '^g', label='Alpha+AOD', zorder=2)
    axs[i][j].set_title(method)
    axs[i][j].legend(loc=9)
    axs[i][j].axhline(80, color='k')
    axs[i][j].axhline(120, color='k')
...
for key1 in ['Cd']:
    ...
    for key2 in list(matrix.keys())[:-7]:
        ...
        axs[i][j].plot(matrix_nooutliers[key1].where(
            matrix_nooutliers['Cd'] < 0.008), matrix_nooutliers[key2], 'o')
        axs[i][j].set_xlabel(key1)
        axs[i][j].set_ylabel(key2)
        j += 1
        if j % 8 == 0:
...
x = 'Na sol'
y = 'Cl'
ax.plot(matrix[x], matrix[y].where(
    events['Event'].isin(['S', 'SP', 'SN'])), 'o')
ax.plot(matrix[x], matrix[y].where(
    ~events['Event'].isin(['S', 'SP', 'SN'])), 'x')
ax.set_xlabel(x)
ax.set_ylabel(y)
plt.show()

fig, ax = plt.subplots()
ax.plot(matrix['NH4'], matrix['NO3'].where(
    ~events['Event'].isin(['S', 'SP', 'SN'])), 'o')
ax.plot(matrix['NH4'], matrix['NO3'].where(
    events['Event'].isin(['S', 'SP', 'SN'])), 'd')
ax.set_xlabel('NH$_4$')
ax.set_ylabel('NO$_3$')
plt.show()

fig, ax = plt.subplots()
ax.plot(matrix['PM2.5'], matrix['EC'].where(
    ~events['Event'].isin(['S', 'SP', 'SN'])), 'o')
ax.plot(matrix['PM2.5'], matrix['EC'].where(
    events['Event'].isin(['S', 'SP', 'SN'])), 'd')
...
fig, ax = plt.subplots()
ax.plot(matrix['PM2.5'], matrix['OC'].where(
    ~events['Event'].isin(['S', 'SP', 'SN'])), 'o')
ax.plot(matrix['PM2.5'], matrix['OC'].where(
    events['Event'].isin(['S', 'SP', 'SN'])), 'd')
ax.set_xlabel('PM$_{2.5}$')
...
ax.plot([0, 1.2], 1/0.55661 * np.array([0, 1.2]),
        label=f'Bibliografia, Cl/Na = {1/0.55661:.3g}')
# ax.plot([0, 1.2], slope * np.array([0, 1.2]) + intercept, label=f'{slope:.3g} Na + {intercept:.3g}')
ax.plot([0, 1.2], 1/0.55661 * np.array([0, 1.2])+intercept,
        label=f'Bibliografia - intercept, Cl/Na = {1/0.55661:.3g}')
...
ax.plot([0, 1.2], 1/0.55661 * np.array([0, 1.2]),
        label=f'Bibliografia, Cl/Na = {1/0.55661:.3g}')
ax.plot([0, 1.2], 35.453/22.9898 * np.array([0, 1.2]),
        label=f'Bibliografia, Cl/Na = {35/23:.3g}')
...
fig, ax = plt.subplots()
ax.plot(matrix['K'], matrix['ECPk1 C'].where(
    events['Event'].isin(['S', 'SP', 'SN'])), 'o')
ax.set_xlabel('K')
ax.set_ylabel('OC')
plt.show()
...
ax.errorbar(matrix.index, matrix[poll], yerr=unc[poll],
            marker='X', label=poll, capsize=2, capthick=2)
ax.errorbar(matrix.index, matrix[poll2], yerr=unc[poll2],
            marker='X', label=poll2, capsize=2, capthick=2)
